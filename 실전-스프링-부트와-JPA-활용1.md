   ...
   - 엔티티 설계시 주의점
     ...
     - **모든 연관관계는 지연로딩으로 설정한다!!!**
       ...
       - `N+1 문제`
         ...
         - Member를 조회하는 쿼리를 호출한다 ==> Order 조회 쿼리가 Member의 조회 데이터(row) 개수만큼 로그에 발생한다 
         - 즉 Member의 데이터 개수가 100개이고/ Member 데이터를 조회할 때, Member 데이터 조회를 위한 쿼리 1개랑 Order를 조회하는 100개(Member의 데이터 개수)의 쿼리가 실행되어서 총 1+100개의 쿼리가 로그에 출력된다. 이를 `N+1 문제`라고 한다 
